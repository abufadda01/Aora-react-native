...
export const getLatestPosts = async () => {
    try {
        const posts = await dbs.listDocuments(appwriteConfig.databaseId , appwriteConfig.videoCollectionId , [Query.orderDesc("$createdAt" , Query.limit(7))])
        return posts.documents
    } catch (error) {
        console.log(error)
    }
...
export const searchPosts = async (query) => {
    try {
        const posts = await dbs.listDocuments(appwriteConfig.databaseId , appwriteConfig.videoCollectionId , [Query.search("title" , query)])
        return posts.documents
    } catch (error) {
        console.log(error)
    }
